...
Initial inputs (outputs from DGTal create_distance_map and analyze_graph functions): 
	  data_file.txt = txt file containing node degrees (among other things) 
...
1) [arcs, nodes] = formatData('data_file.txt', 'points_file.txt', 'dmap.nrrd');
...
4) [orientation, newNetwork, connectivity, maxNumDaughters, maxNumParents] = directedGraphInfo(arcs, nodes, path)
